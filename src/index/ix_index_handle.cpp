...
bool IxNodeHandle::leaf_lookup(const char *key, Rid **value) {
    ...
    if (idx != get_size() && ix_compare(key, get_key(idx), file_hdr->col_types_,
                                        file_hdr->col_lens_) == 0) {
...
void IxNodeHandle::insert_pairs(int pos, const char *key, const Rid *rid,
    ...
    // 4. 更新当前节点的键数量
    if (!get_size() || pos <= get_size()) {
        auto tot_len = file_hdr->col_tot_len_;
        auto key_len = tot_len * n, last_key_len = tot_len * (get_size() - pos);
        auto rid_len = sizeof(Rid) * n,
             last_rid_len = (get_size() - pos) * sizeof(Rid);

        memmove(get_key(pos + n), get_key(pos), last_key_len);
        memmove(get_rid(pos + n), get_rid(pos), last_rid_len);

        memcpy(get_key(pos), key, key_len);
        memcpy(get_rid(pos), rid, rid_len);
...
int IxNodeHandle::insert(const char *key, const Rid &value) {
    ...
    // 4. 返回完成插入操作之后的键值对数量
    int idx = lower_bound(key);
    if (!get_size() || ix_compare(key, get_key(idx), file_hdr->col_types_,
                                  file_hdr->col_lens_) != 0) {
...
void IxNodeHandle::erase_pair(int pos) {
    ...
    if (pos < get_size()) {
        auto tot_len = file_hdr->col_tot_len_;
        auto key_len = tot_len * (get_size() - pos - 1);
        auto rid_len = sizeof(Rid) * (get_size() - pos - 1);

        memmove(get_key(pos), get_key(pos + 1), key_len);
        memmove(get_rid(pos), get_rid(pos + 1), rid_len);
...
bool IxIndexHandle::get_value(const char *key, std::vector<Rid> *result,
    ...
    auto leaf_page = find_leaf_page(key, Operation::FIND, transaction);
    Rid **rid = new Rid *;
    if (!leaf_page.first->leaf_lookup(key, rid)) {
